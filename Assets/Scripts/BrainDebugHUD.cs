...
/// <summary>
/// Отладочный HUD для RobotBrain: показывает все 15 наблюдений, текущие действия
...
public class BrainDebugHUD : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        sb.AppendLine("<b>OBSERVATIONS (15)</b>");
        ...
        sb.AppendLine(Row("11 dX from start  ", Fmt(brain.Obs11_DxNorm),            "white"));
        sb.AppendLine(Row("12 dZ from start  ", Fmt(brain.Obs12_DzNorm),            "white"));
        sb.AppendLine(Row("13 Heading        ", Fmt(brain.Obs13_HeadingNorm),       "white"));
        sb.AppendLine(Row("14 Speed          ", Fmt(brain.Obs14_Speed),             "white"));
    ...
    private static string GripLabel(int cmd)
    {
        switch (cmd)
        {
            case 1:  return " (GRIP)";
