...
def force_free_port(port):
	"""
	Принудительно освобождает TCP-порт перед bind(): находит PID, который его
	слушает (через /proc, без внешних утилит вроде fuser/lsof - их нет в образе),
	и убивает его SIGKILL. Нужно, чтобы "зависший" процесс с прошлого запуска
	(например, unity_master_team1.py, вручную перезапущенный внутри того же
	контейнера для отладки, пока фоновый экземпляр ещё жив) не мешал текущей
	инициализации занять порт заново.
	"""
	try:
		with open('/proc/net/tcp', 'r') as f:
			lines = f.readlines()[1:]
	except Exception as e:
		print(f"[xr_config] force_free_port({port}): не смог прочитать /proc/net/tcp: {e}")
		return
	...
	for line in lines:
		...
		if hex_port.upper() == port_hex and state == '0A':  # 0A = TCP_LISTEN
			listen_inodes.add(inode)

	if not listen_inodes:
		return  # порт свободен, освобождать нечего
	...
	for pid_str in os.listdir('/proc'):
		...
		for fd in fds:
			...
			if link.startswith('socket:[') and link[8:-1] in listen_inodes:
				pid = int(pid_str)
				print(f"[xr_config] Порт {port} занят PID {pid} - принудительно освобождаю (SIGKILL)")
				try:
	...
	if killed_any:
		time.sleep(0.3)  # дать ядру время реально закрыть сокет перед нашим bind()
...
# Порт 2005 не биндится нигде в этом файле, но исторически конфликтовал с
# unity_master_team1.py (см. комментарий в unity_gripper_ir_team1.py и
# monkey-patch в заброшенном unity_master_team1.py.save) - вероятно, старый
# номер порта BT_SERVER до переименования в 2002, либо порт занят чем-то ещё на
# этой же машине. На всякий случай принудительно освобождаем его тоже, ещё до
# наших собственных bind() - если сейчас на нём ничего нет, просто no-op.
force_free_port(2005)

# 蓝牙服务端参数设置
# ПАТЧ: bind() был безусловным - если порт уже занят (например, "зависший"
# процесс unity_master_team1.py от предыдущего запуска, не убитый перед новым
# docker exec), исключение здесь роняет ИМПОРТ xr_config, а значит и ВСЕ модули,
# которые его тянут (xr_ultrasonic -> xr_servo/xr_socket/xr_motor), из-за
# функциональности (BT/TCP пульт), которая в ROS/Unity-пайплайне вообще не
# используется. force_free_port() убивает того, кто держит порт, ДО bind() -
# текущая инициализация всегда забирает порт себе. SO_REUSEADDR остаётся как
# вторая линия защиты (TIME_WAIT после SIGKILL).
force_free_port(2002)
BT_SERVER = socket(AF_INET, SOCK_STREAM)
try:
	BT_SERVER.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
	BT_SERVER.bind(('', 2002))		# 蓝牙绑定2002端口
	BT_SERVER.listen(1)
except OSError as e:
	print(f"[xr_config] WARNING: не удалось забиндить Bluetooth-порт 2002 даже после force_free_port ({e}) - BT-пульт (не используется в ROS/Unity) недоступен, остальные драйверы продолжают загрузку")

# TCP服务端参数设置
force_free_port(2001)
...
try:
	...
	print(f"[xr_config] WARNING: не удалось забиндить TCP-порт 2001 даже после force_free_port ({e}) - WiFi-пульт (не используется в ROS/Unity) недоступен, остальные драйверы продолжают загрузку")
